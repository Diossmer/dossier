...
export function HeroPortada({ title, data, phase, isActive }: SlideProps) {
  ...
  return (
    ...
    >
      ...
      <p className="text-dossier-subheading text-gray-300 max-w-2xl mb-4">
        {data.subtitle as string}
      </p>

      <div className="h-px w-32 bg-gradient-to-r from-transparent via-blue-400 to-transparent mb-6" />

      <p className="text-dossier-body text-gray-400">
        {data.tagline as string}
      </p>

      <div className="mt-8 flex flex-col items-center gap-2 text-dossier-caption text-gray-500">
        <span>{data.group as string}</span>
        <span>{data.date as string}</span>
      </div>

      {isActive && (
        <div className="absolute bottom-8 text-gray-600 animate-float">
